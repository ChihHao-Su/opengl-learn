...
struct ImGuiIO
{
    ...
    ImVec2      DisplayFramebufferScale;        // = (1, 1)         // For retina display or other situations where window coordinates are different from screenFrameBuffer coordinates. This generally ends up in ImDrawData::FramebufferScale.
...
struct ImDrawData
{
    ...
    IMGUI_API void  ScaleClipRects(const ImVec2& fb_scale); // Helper to scale the ClipRect field of each ImDrawCmd. Use if your final output buffer is at a different scale than Dear ImGui expects, or if there is a difference between your window resolution and screenFrameBuffer resolution.
};
...
enum ImGuiViewportFlags_
{
    ...
    ImGuiViewportFlags_NoRendererClear          = 1 << 8,   // Platform Window: Renderer doesn't need to clear the screenFrameBuffer ahead (because we will fill it entirely).
...
struct ImGuiPlatformIO
{
    ...
    void    (*Renderer_RenderWindow)(ImGuiViewport* vp, void* render_arg);  // . . . R .  // (Optional) Clear screenFrameBuffer, setup render target, then render the viewport->DrawData. 'render_arg' is the value passed to RenderPlatformWindowsDefault().
